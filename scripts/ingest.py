...
# Load csv files
db_con.sql(r"CREATE TABLE bike_data AS SELECT * FROM read_csv_auto('data\bike\*.csv.gz', union_by_name=True, filename=1);")
db_con.sql(r"CREATE TABLE central_park_weather AS SELECT * FROM read_csv_auto('data\central_park_weather.csv', filename=1);")
db_con.sql(r"CREATE TABLE daily_citi_bike_trip_counts_and_weather AS SELECT * FROM read_csv_auto('data\daily_citi_bike_trip_counts_and_weather.csv', filename=1);")
db_con.sql(r"CREATE TABLE fhv_bases AS SELECT * FROM read_csv_auto('data\fhv_bases.csv', filename=1);")
